...
void gpio_configure_skx_pads(const struct skx_pad_config *gpio, size_t num)
{
	...
	struct skx_pad_config *GpioData;
	GPIO_PAD Group;
	uint32_t GroupIndex;
	uint32_t PadNumber;
	uint32_t FinalValue;
	...
	for (Index = 0; Index < (uint32_t)num; Index++) {
		...
		GpioData = (struct skx_pad_config *)&(gpio[Index]);
